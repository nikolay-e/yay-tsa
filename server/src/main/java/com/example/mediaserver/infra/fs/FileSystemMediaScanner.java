package com.example.mediaserver.infra.fs;

import com.example.mediaserver.infra.persistence.entity.*;
import com.example.mediaserver.infra.persistence.repository.*;
...
public class FileSystemMediaScanner {
    ...
    public ScanResult scan(String libraryRoot) {
        ...
        log.info("Scan completed: {} scanned, {} added, {} updated, {} removed, {} errors",
            audioFiles.size(), added.get(), updated.get(), removed, errors.get());
    ...
    private void processAudioFile(
        ...
        try {
            ...
            OffsetDateTime mtime = OffsetDateTime.ofInstant(attrs.lastModifiedTime().toInstant(), ZoneOffset.UTC);
            ...
            if (existingItem.isPresent()) {
                ...
                if (item.getMtime() != null && item.getMtime().equals(mtime) &&
                    item.getSizeBytes() != null && item.getSizeBytes().equals(fileSize)) {
                    return;
                }
    ...
    private ItemEntity findOrCreateArtist(String artistName, String libraryRoot) {
        final String finalArtistName = (artistName == null || artistName.isBlank())
            ? "Unknown Artist"
            : artistName;
    ...
    private ItemEntity findOrCreateAlbum(String albumName, ItemEntity artistItem, String libraryRoot) {
        final String finalAlbumName = (albumName == null || albumName.isBlank())
            ? "Unknown Album"
            : albumName;
    ...
    private void processGenres(ItemEntity item, List<String> genres) {
        ...
        for (String genreName : genres) {
            ...
            if (!exists) {
                ItemGenreEntity.ItemGenreId id = new ItemGenreEntity.ItemGenreId(item.getId(), genre.getId());
    ...
    private List<Path> collectAudioFiles(Path root, Set<String> extensions, Set<String> ignoredFolders) {
    ...
    private AudioMetadata createFallbackMetadata(Path filePath) {
        ...
        return new AudioMetadata(
            filename, null, null, null, null, null, null,
            0L, 0, 0, 2, "unknown", null, null,
            List.of(), null, null
        );
    }
