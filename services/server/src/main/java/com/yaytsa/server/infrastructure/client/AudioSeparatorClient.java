...
public class AudioSeparatorClient {
  ...
  public record SeparationResult(
      String instrumentalPath, String vocalPath, long processingTimeMs) {}
  ...
  public SeparationResult separate(Path audioFilePath, String trackId) {
    ...
    var request = Map.of("inputPath", audioFilePath.toString(), "trackId", trackId);
    ...
    for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        log.info(
            "Audio separation completed for track {} in {}ms (attempt {})",
            trackId,
            elapsed,
            attempt);

        return new SeparationResult(
            response.get("instrumental_path"), response.get("vocal_path"), elapsed);
      } catch (ResourceAccessException e) {
        lastException = e;
        if (isRetryableException(e) && attempt < MAX_RETRIES) {
          long delay = INITIAL_RETRY_DELAY_MS * (1L << (attempt - 1));
          log.warn(
              "Audio separation attempt {} failed for track {} (retrying in {}ms): {}",
              attempt,
              trackId,
              delay,
              e.getMessage());
          ...
          log.error(
              "Audio separation failed for track {} after {} attempts: {}",
              trackId,
              attempt,
              e.getMessage());
    ...
    throw new RuntimeException(
        "Audio separation failed after " + MAX_RETRIES + " attempts", lastException);
  }
