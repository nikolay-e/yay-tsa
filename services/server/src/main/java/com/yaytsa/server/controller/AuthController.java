...
public class AuthController {
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully authenticated",
                    content = @Content(schema = @Schema(implementation = AuthenticationResultResponse.class))),
    ...
    public ResponseEntity<AuthenticationResultResponse> authenticateByName(
        ...
        try {
            AuthService.AuthenticationResult result = authService.authenticateByName(
                request.username(),
                request.password(),
                deviceId,
                deviceName,
            ...
            SessionInfoResponse sessionInfo = SessionInfoResponse.forAuth(
                result.sessionId(),
            ...
            AuthenticationResultResponse response = AuthenticationResultResponse.create(
                userDto,
                sessionInfo,
                result.accessToken()
            );

            log.info("User authenticated successfully: {} from device: {}",
                result.user().getUsername(), result.deviceId());
