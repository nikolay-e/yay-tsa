...
public class AcoustIdService {
  ...
  @Value("${yaytsa.metadata.acoustid.api-key:}")
  private String apiKey;

  public AcoustIdService(ObjectMapper objectMapper) {
    ...
    try {
      ...
      String body =
          "client="
              ...
              + duration
              + "&fingerprint="
              + URLEncoder.encode(fingerprint, StandardCharsets.UTF_8)
      ...
      log.info(
          "AcoustID lookup: duration={}s, fingerprint={}chars", duration, fingerprint.length());
  ...
  private Optional<AcoustIdResult> selectBestResult(List<AcoustIdLookupResult> results) {
    for (AcoustIdLookupResult result : results) {
      ...
      for (AcoustIdRecording recording : result.recordings) {
        ...
        return Optional.of(new AcoustIdResult(artist, title, album, year, recording.id));
      }
    }
  ...
  @JsonIgnoreProperties(ignoreUnknown = true)
  record AcoustIdLookupResult(String id, Double score, List<AcoustIdRecording> recordings) {}
  ...
  @JsonIgnoreProperties(ignoreUnknown = true)
  record AcoustIdRelease(String id, AcoustIdReleaseDate date) {}
