...
public class ItemsController {
    ...
    public ResponseEntity<QueryResultResponse<BaseItemResponse>> getItems(
        ...
        List<BaseItemResponse> dtos = itemsPage.getContent().stream()
            .map(item -> convertToDto(item, userUuid, enableUserData))
            .collect(Collectors.toList());

        QueryResultResponse<BaseItemResponse> result = new QueryResultResponse<>(
            dtos,
            Math.toIntExact(itemsPage.getTotalElements()),
            startIndex
        );
    ...
    public ResponseEntity<QueryResultResponse<BaseItemResponse>> getAlbumTracks(
        ...
        List<BaseItemResponse> dtos = tracks.stream()
            .skip(startIndex)
            .limit(limit)
            .map(track -> {
