...
public class GeniusProvider implements MetadataProvider {
  ...
  @Value("${yaytsa.metadata.genius.access-token:}")
  private String accessToken;

  public GeniusProvider(ObjectMapper objectMapper) {
  ...
  public Optional<EnrichedMetadata> findMetadata(String artist, String title) {
    ...
    try {
      ...
      String encodedQuery =
          URLEncoder.encode(searchQuery, StandardCharsets.UTF_8).replace("+", "%20");
      ...
      GeniusSearchResponse searchResponse =
          objectMapper.readValue(response.body(), GeniusSearchResponse.class);
      ...
      log.info(
          "Genius returned {} hits for: {} - {}",
          searchResponse.response.hits.size(),
          artist,
          title);
        ...
        if (confidence >= 0.70) {
          ...
          if (song.id != null) {
            try {
              ...
              HttpResponse<String> detailResponse =
                  httpClient.send(detailRequest, HttpResponse.BodyHandlers.ofString());
              GeniusSongResponse songResponse =
                  objectMapper.readValue(detailResponse.body(), GeniusSongResponse.class);

              if (songResponse != null && songResponse.response != null) {
                GeniusSongDetail detail = songResponse.response.song;
                log.debug(
                    "Got detailed song response, album: {}",
                    detail.album != null ? detail.album.name : "null");
          ...
          if (albumId != null) {
            try {
              ...
              HttpRequest albumTracksRequest =
                  HttpRequest.newBuilder()
                      ...
                      .GET()
                      .build();
              HttpResponse<String> albumTracksResponse =
                  httpClient.send(albumTracksRequest, HttpResponse.BodyHandlers.ofString());
              GeniusAlbumTracksResponse albumTracks =
                  objectMapper.readValue(
                      albumTracksResponse.body(), GeniusAlbumTracksResponse.class);
              if (albumTracks != null
                  && albumTracks.response != null
                  && albumTracks.response.tracks != null) {
          ...
          String artistName = song.primaryArtist != null ? song.primaryArtist.name : artist;

          // Use song art image from Genius (cached behind auth — same model as Plex/Navidrome)
          String coverArtUrl =
              song.songArtImageUrl != null ? song.songArtImageUrl : song.headerImageUrl;
  ...
  private String cleanAlbumName(String albumName) {
    ...
    return albumName.replaceAll("\\s*\\([^)]*\\)\\s*", " ").replaceAll("\\s+", " ").trim();
  }
  ...
  @JsonIgnoreProperties(ignoreUnknown = true)
  record GeniusArtist(Long id, String name, @JsonProperty("image_url") String imageUrl) {}
  ...
  @JsonIgnoreProperties(ignoreUnknown = true)
  record GeniusSongDetail(GeniusAlbum album, @JsonProperty("release_date") String releaseDate) {}
