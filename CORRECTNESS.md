...
## 2026-06-21 · c0c8efa2 · full-repo correctness sweep (12 scouts → 6 adversarial verifiers → consolidation)

Deterministic layer GREEN before audit: frontend `tsc -b` ✅, backend `compileKotlin compileTestKotlin spotlessCheck` ✅ — the entire "non-existent API / wrong signature / fake package" class is cleared on both stacks; 12 read-only scouts + 6 refute-first verifiers checked LOGIC + dynamic refs only. Every finding below carries a verifier verdict (CONFIRMED at file:line, or REFUTED/document).

### 🔴 Must-fix (confirmed wrong behavior in normal usage)

1. **DO** — `adapter-jellyfin/.../JellyfinAdaptiveController.kt:188` — Radio/adaptive `refreshQueue` bootstrap gates ML-similar seeds with `musicSurfaceFilter.filterRedLines(tracks, uid)` (red-line ONLY) instead of `.filter(...)`. ML embeds **every** track incl. audiobooks (`MlFeatureExtractor` has no genre filter; `findSimilarByClap/Mert/Discogs` is pure pgvector kNN, no genre predicate), so an audiobook acoustically near the seed leaks into the music queue. Lone outlier — every other path (406/410/428/484/568) uses `filter()`/`isBlocked()`. Fix: `filterRedLines` → `filter`.
2. **DO** — `adapter-jellyfin/.../JellyfinItemsController.kt:462-500` (`buildPersonalizedTracks`, reached :236-238) — the default personalized homepage feed (`SortBy=DatePlayed`, the PWA Daily Mix path) returns affinity + favorites + `browseTracksRandom` with **no** `musicSurfaceFilter` at all. Reachable whenever `excludedGenres.isEmpty()` (which this branch requires). A favorited/listened audiobook leaks straight into the Audio surface. Verifier upgraded 🟡→🔴 (default feed, not ML-coincidence-gated). Fix: wire `musicSurfaceFilter` into the controller and filter the personalized output.
3. **DO** — `infra-ml-worker/.../AffinityAggregator.kt:47,68-108` — incremental watermark is `SELECT MAX(updated_at) FROM user_track_affinity`, but `updated_at` is written `= now()` (aggregation wall-clock) while the signal filter is `WHERE ps.created_at > watermark` (signal emit-time = `ctx.requestTime`). Two different clocks: a signal whose `created_at <= now()(run N)` but not yet visible to run N's SELECT (READ COMMITTED late-commit / clock skew) fails `created_at > watermark` on run N+1 and is **silently dropped forever** — the UPSERT folds additive deltas (`affinity_score = uta.affinity_score + EXCLUDED…`), never recomputing from history, so a skipped signal is permanently lost. Blast radius: every affinity metric feeding the LLM-DJ taste profile. Fix: derive the watermark from consumed signal data (`MAX(ps.created_at)` of the batch, already computed at :93) and/or add a safety lookback window — never `now()`.

### 🟡 Fixing this pass (confirmed, cheap, real)

1. **DO** — `core-domain/playback/.../PlaybackHandler.kt:224` — `resetPos = cmd.entryId != null && cmd.entryId != s.currentEntryId` keys off the raw command, not the resolved entry. After `RemoveFromQueue`-of-current (leaves `currentEntryId=null`, STOPPED, non-zero `lastKnownPosition` snapshot — proven by `PlaybackHandlerTest.kt:247`), a `Play(null)` resolves `queue.first()` but `resetPos==false` → the fresh track resumes at the previous track's stale offset. The 7e43e923 fix touched line 233 only, not the predicate. Fix: `resetPos = resolved != s.currentEntryId`.
2. **DO** — `adapter-jellyfin/.../JellyfinAdaptiveController.kt` — five write endpoints (`updateState:119`, `endSession:132`, `refreshQueue`/RewriteQueueTail:230, `recordSignal:252`, `updatePreferences:310`) discard `CommandResult` and return 204 unconditionally → a genuine OCC `Conflict`/`InvariantViolation` is a silent dropped write reported as success. The `currentSessionVersion` pre-read narrows but does not close the race (doc-comment :145-152 admits the swallow). Fix: inject `HttpFailureTranslator`, route `Failed → failureTranslator.empty(...)`.
3. **DO** — `adapter-jellyfin/.../JellyfinDtos.kt:15-53` — dead duplicate local `BaseItem` (missing `ProductionYear`/`IndexNumber`/`ParentIndexNumber`), `UserItemData`, `NameIdPair`; all controllers import the _shared_ `dev.yaytsa.adaptershared.BaseItem` (which has those fields). Local copies are unreferenced and drifted → latent trap. Fix: delete the three dead DTOs, keep live ones.
4. **DO** — `apps/web/src/shared/ui/GlobalSearchBar.tsx:64` — `onChange` passes `{replace:true}` **unconditionally**; the first keystroke typed from a non-`/search` page (e.g. `/albums`) replaces the originating route with `/search?q=a`, so Back can't return to it. (Two scouts disagreed; verifier confirmed the second is right — `onSearchRoute` is computed at :10 but never consulted in `submit`.) Fix: `submit(value, { replace: onSearchRoute })`.
5. **DO** — `apps/web/src/features/library/hooks/useReorderFavorites.ts:31,42-55` — optimistic reorder builds `reordered` from `itemIds` only and slices it back across pages; any cached item not in `itemIds` is dropped + trailing pages truncate. `onMutate` iterates the partial key `['albums']` matching the **unfiltered** favorites cache, so reordering a **search-filtered** favorites list rewrites the full cache with a subset → visible favorites silently truncated until `onSettled` refetch. Only the `<0.5` gate stands between this and guaranteed corruption. Fix: scope the optimistic write to the exact query key (or skip optimistic update when `itemIds.length < allItems.length` / a search filter is active).
6. **DO** — `adapter-mcp/.../McpTools.kt:362-369` — `get_album` returns `browseTracksByAlbum` output with no `musicSurfaceFilter`, unlike `search_library`. Audiobook marker is **track-scoped** (`isAudiobookTrack` reads `track.genre`, populated identically here), so the leak is real (low blast radius — caller names a specific album). Fix: apply `musicSurfaceFilter.filter(tracks, userId)`.
7. **DO** — `packages/core/src/api/api.client.ts:227` — `parseResponse` returns `undefined` when `!contentType?.includes('json')`, so a 200 with a real body but absent/abbreviated/proxy-stripped Content-Type surfaces as a spurious "Empty response" throw (`items.service.ts:58`, `api.client.ts:506`). Latent (backend sends `application/json`), cheap to harden. Fix: gate empty-detection on status/`content-length` only; attempt `response.json()` in try/catch independent of the Content-Type substring.

### 🔵 Cheap consistency / honesty fixes (this pass)

1. **DO** — `JellyfinAdaptiveController.startSession:105` + `JellyfinAdminController.createUser:107` map ALL `CommandResult.Failed → 400`, bypassing `HttpFailureTranslator` (an OCC-race `Conflict` should be 409). Route both through the translator.
2. **DO** — `core-application/playback/.../ScrobbleService.kt:13,28-29` — "completed if elapsed > 50% of position" compares **wall-clock** `elapsed` (startedAt→stoppedAt) against half the **in-track** `positionMs` (different clocks), so a long pause marks a barely-played track completed. Comment is literally accurate (→ smell, not lie). Fix: compare position vs track **duration** (`positionMs > runTimeMs/2`); keep the 240s wall-clock floor as a coarse backstop.

### Document — NOT a bug (verifier-killed)

- **Subsonic token-auth rejected (`SubsonicAuthFilter.kt:48`, code 41).** Passwords are BCrypt strength-13 at rest; MD5(password+salt) is structurally unverifiable without plaintext. Rejecting `t`/`s` token-auth with spec-code 41 is the **correct** posture — weakening BCrypt to support DSub-style token auth would be the real bug. Clients should use `apiKey` or `p=`/`enc:` password auth. Decision, not fix.

### Defer (latent / intended / leverage, not correctness-blocking)

- `useInfiniteLibraryQuery.ts:55-63` forward(actual-count)/backward(fixed-limit) cursor asymmetry — 🔵 latent; backward pagination is effectively unused (only `fetchNextPage` fires).
- `PlayerBar.tsx:145,189-198` local transport bypasses `groupSync.sendAction` in group mode — confirmed **intended** remote-schedule-driven design; the live-but-meaningless buttons are a UX-guard gap (feature decision), not a correctness bug.
- `MlFeatureExtractor.kt:45` `findAll()` slurps every embedding row to build an id-set (contradicts the line-47 paging comment) — 🔵 perf/leverage, no correctness impact.
- Subsonic `getAlbumList type=frequent/highest` collapse to created-desc (`SubsonicController.kt:335`) — no play-count/rating album projection exists; honest approximation on a compat surface. Document; build the projection only if a client needs it.

### Verified-correct (feared bugs ABSENT, confirmed at HEAD)

Domain OCC/idempotency/lease invariants uniform; pgvector cosine `<=>` ↔ `vector_cosine_ops` HNSW ↔ `vector(N)` ↔ ASC nearest-first; outbox `FOR UPDATE SKIP LOCKED` REQUIRES_NEW; scanner empty-walk guard + >50% mass-delete breaker + rename-preserves-entity-id; `MediaPathSafety` NOFOLLOW_LINKS TOCTOU; karaoke gain math (instrumental@1.0 / vocals variable, not swapped) + NaN-blend guard + stalled-primary watchdog guard (prior fixes held); audio promise-chain cancellation; PlaybackQueue Fisher-Yates + idempotent-only retry + full-jitter backoff; group-sync reconnect `reconcileSnapshot` epoch-bypass (prior fix held); `useRemoteCommands` typeof-guard + bounded commandId dedup (prior fixes held); SW Range/206/416 + LRU eviction; Dockerfile version-inject in builder stage; CronJob tag pinning.

_Topology: 12 read-only scouts → 6 refute-first verifiers → 1 consolidation. Fix phase follows._

### Fix-phase outcome (same commit)

Applied all 🔴/🟡 + the two cheap 🔵 (consistency/honesty) fixes via 6 file-disjoint executors. Gates: frontend `type-check`+`build` ✅; backend `./gradlew build` (compile + spotlessCheck + Testcontainers, 202 tests) ✅ after one test-isolation correction.
