...
- **CI note**: the `Pre-commit Checks` job fails on a cold pre-commit cache because the private `nikolay-e/pre-commit-hooks` repo can't be cloned anonymously on the runner — pre-existing, needs a CI deploy token; does not gate the telemetry E2E (`Web E2E (mocked)`) which is the relevant gate here.
