...
## QA Session Findings (2026-06-12)

- **Backend restarted on EVERY push to the repo (even frontend/ci-only commits), killing active streams.** `charts/yay-tsa-v2` deployment hashed the whole rendered ConfigMap manifest into `checksum/config`; the manifest's `helm.sh/chart`/`app.kubernetes.io/version` labels change with every chart release (Release Helm Chart runs on every push, ArgoCD `targetRevision: >=` picks each one up). Symptom triad: pod replicaset hash changes while the image tag stays identical, `checksum/config` differs between rollout revisions, gitops log shows NO v2 values change. Mid-playback effect: Cloudflare 502 on `/Sessions/Playing/Progress`, stream death → media-error auto-advance to a wrong chapter. Fixed by hashing only the values feeding the ConfigMap data (`print (toYaml .Values.backend.config) (toYaml .Values.backend.database.hikari) | sha256sum`). The frontend chart deliberately keeps manifest-hashing (its data embeds `.Chart.AppVersion`).
- **Playwright `route.fulfill` stream mocks MUST answer Range requests with 206 + `Content-Range`/`Accept-Ranges`** — Chromium silently snaps `audio.currentTime` back to 0 on seek for media served without byte-range support. A seek-on-load e2e against a plain `fulfill({body: WAV})` false-fails with "started from 0" and perfectly masquerades as a prod resume bug (`currentTime set to 1.5` visible in instrumentation, `play() pos=0` after). Shared range-aware stub lives in `tests/e2e/helpers/media-fixtures.ts` usage (audiobooks.mocked.spec).
- **Browser-QA auth injection (avoids master autofill):** in-page `fetch('/api/Users/AuthenticateByName')` then `localStorage.yaytsa_session_persistent` + `yaytsa_user_id_persistent` + **`yaytsa_remember_me='true'`** (without the flag `loadSessionPersistent` returns null) + `yay_token` cookie, then SPA-navigate. Audiobook resume verified live this way: play → full reload kills the player (keepalive flush fires) → Resume lands on the same chapter within heartbeat granularity (~2s).
- **`PlaylistItemId` is the playlist POSITION on the wire** (`getPlaylistItems` emits the index; `EntryIds` removal parses Int positions). The Move endpoint matched track ids only, so every protocol-conformant `POST /Playlists/{id}/Items/{PlaylistItemId}/Move/{n}` 404'd (PWA never calls Move; Subsonic/Jellyfin clients do). Fixed to accept position first, track-id fallback. Regression tests in `PlaylistsIntegrationTest`.
- **`ResponseEntity<String>` without charset in MediaType → ISO-8859-1 body.** Spring's StringHttpMessageConverter defaults to ISO-8859-1 when the content type lacks a charset — Subsonic `/rest/*` JSON came out `;charset=ISO-8859-1` (mangled Cyrillic tags + schemathesis content-type failures). Always build `MediaType(type, Charsets.UTF_8)` on String bodies (`SubsonicResponseWriter.write`).
- **Spring DAO translation re-breaks the malformed-UUID→404 convention.** Repository-layer `UUID.fromString` failures surface as `InvalidDataAccessApiUsageException` (not `IllegalArgumentException`), so the GlobalExceptionHandler 404-mapping needs the same `Invalid UUID string` prefix check in BOTH handlers.
- **`400 problem+json {detail:"Bad Request"}` on paths containing `%3B`/`%2A` is Spring Security StrictHttpFirewall**, rejected before any controller — looks like an app 400 in schemathesis output, fix is NOT app-side (tracked in #226).
- **`sort-package-json` pre-commit hook fails CI after hand-editing root `package.json`** (e.g. `overrides` for security bumps) — run `npx sort-package-json package.json` before push.
- **Maven Central 403 in `v2 Build Docker`** (`Could not GET ... kotlin-stdlib ... 403 Forbidden`) is a shared-runner rate-limit flake; `gh run rerun <id> --failed`. The Gradle test job passes because its deps are action-cached; only the fresh container build hits Central.
- **1Password SSH signer lock mid-session**: `git commit` → `error: 1Password: failed to fill whole buffer` + `ssh-add -l` → "agent has no identities". All commits block until the user unlocks 1Password; keep working (gh CLI, kubectl, edits) and batch the commits.
- **SonarCloud noise (documented, don't chase):** `S3735 void operator` CRITICALs conflict with the repo's `no-floating-promises` fire-and-forget idiom (`void page.route(...)`, `void playTracks(...)`); `S7766 Prefer Math.max` flagged on a STRING-comparison reduce in `useAudiobooks.lastUpdatedAt` (Math.max on ISO strings is wrong).
- **Declaring the real id alphabet on OpenAPI path params kills the biggest schemathesis FP class.** `OpenApiConfig.pathIdPatternCustomizer` sets `pattern` on every plain-string path param, so positive fuzzing stops generating `;`-garbage that StrictHttpFirewall rejects as 400 ("API rejected schema-compliant request"). After it deployed, the failure count converged 5→3 and the only residuals are container-level (Tomcat pre-servlet `text/html` 400 on invalid 4-byte unicode) — gate-policy fix tracked in nikolay-e/autoqa#11; do NOT chase those in app code.
- **Crawler treats a 404 cover as a NEW blocking finding even when the album genuinely has no art** (no cover file, `ffprobe` shows no `attached_pic` in any chapter — «Птица малая»). The embedded-tag fallback (`EmbeddedArtworkService`) serves albums whose art lives only in ID3/FLAC tags («Спин» case), but an art-less album correctly 404s; gate-policy fix also in nikolay-e/autoqa#11.
- **Audiobook rips often carry art ONLY in tags** — check with `kubectl exec <backend> -- ffprobe -show_streams <file> | grep attached_pic` before treating an Images 404 as a data bug. Fallback chain: image row → embedded art of the item's own file (or first chapter for albums) → 404.

## QA Session Findings (2026-06-13)

- **A stale `REVIEW_SECURITY.md` / `/review-security` artifact in the repo root must be verified against current HEAD before acting.** A multi-round security audit dropped a 770-line markdown listing an IDOR cluster (favorites/playlists/adaptive/items), MCP-IDOR, GROUP-1, PATH-1, CRYPTO-1 MD5/plaintext fallback, ACTUATOR-1 and SF-4 as live. Nearly all were already remediated the SAME day by a single large `fix(core): Enforce principal authz, protocol fidelity, scanner safety` commit (adds `AuthorizationBoundariesIntegrationTest`, `networkpolicy.yaml`, principal-only identity on every write adapter, removes the Subsonic MD5/plaintext `else`, gates the image endpoint with `MediaPathSafety`). Re-fixing them blindly would have been pure churn. Spot-verify pattern: `grep -n "userId != principal.name" <controller>`, `grep -n "resolveServableFile" JellyfinMediaController.kt`, `grep else SubsonicAuthFilter.kt`. Only one finding (UPLOAD-1) genuinely survived.
- **UPLOAD-1 (real, fixed this pass):** the unimplemented `/tracks/upload` endpoint (returns 501) still wrote the multipart body to a never-deleted temp dir (disk-exhaustion DoS by any authed user) AND returned the absolute server `tempPath` in the response. Fix: drop the file write entirely and the `tempPath` field; keep the 501 + size/name echo.
- **TOCTOU-1 in `MediaPathSafety` is a non-applicable audit rec — do NOT apply `toRealPath(NOFOLLOW_LINKS)`.** The suggested hardening would retain symlinks in the resolved path while `safeRoot` is canonicalized via `toRealPath()` (follows links), so a library root reached through a symlink (`/media`→`/mnt/t7-music`, the prod layout) would fail `startsWith(safeRoot)` → 404 on ALL media. The current code is already safe: it returns the canonicalized `real` path and re-checks `startsWith(safeRoot)` AFTER resolution, and callers read from `real`, not the candidate — no check/use gap. Two copies exist (`infra-media/` for Subsonic, `adapter-jellyfin/` for Jellyfin); both behave identically.
- **post-deploy-qa crawler-404-cover gating finding is the art-less «Птица малая» FP** (re-confirmed live: `GET /Items/e5ff195a…/Images/Primary` → bare 404, item is a MusicAlbum with no cover row and no embedded art). Reproduce the gating findings against the LIVE deployed image before chasing — the failing post-deploy-qa run tests the PREVIOUS backend (deploy lag), and the schemathesis "timeout" on `DELETE /Playlists/-58800/Items` is a transient CF blip (fast 400 against live).
...
## QA Session Findings (2026-05-27)

- **401/403 from the Spring Security filter chain did NOT emit `application/problem+json` — real schema-conformance bug, not the old #8 false positive.** The newer autoqa pin (`cdd60fed`) validates the response _body_ against the documented media type, not just the status code. It found 113 schemathesis failures (`JSON deserialization error` + `Missing Content-Type header`) plus ZAP `Unexpected Content-Type x757`, all on auth-required endpoints. Root cause: `SecurityConfig.authenticationEntryPoint` called `response.sendError(401, ...)`, which (depending on whether `DispatcherType.ERROR` is permitted) yields either an empty body or Spring Boot's default `{"timestamp","status","error","path"}` `application/json` page — neither matches the OpenAPI-documented `application/problem+json`. The `@RestControllerAdvice` `GlobalExceptionHandler` produces correct RFC7807, but it runs at the DispatcherServlet, _after_ the security filters, so filter-level 401/403 never reached it. Fix: a single `ProblemDetailSecurityHandler` bean implementing both `AuthenticationEntryPoint` and `AccessDeniedHandler`, writing `application/problem+json` with `{type,title,status,detail,instance}` directly to the response (bypassing the ERROR dispatch). Wired via `exceptionHandling { authenticationEntryPoint(...); accessDeniedHandler(...) }`. This **supersedes the #8 "401 is fine, schema documents it" note** — the status was documented, the body was not. Regression guard: `AuthIntegrationTest."unauthenticated request returns RFC7807 problem detail body"` asserts `application/problem+json` + `"status":401` + `"title":"Unauthorized"`.
- **ZAP WARN-NEW "A Server Error response code was returned [100000] x20"** appeared in the same run (non-blocking baseline). Likely active-scan fuzzing or Cloudflare 502 (per the documented CF-throttle note) — re-check against fresh backend logs in the post-deploy-qa that runs after the RFC7807 fix deploys; the pod that served the 19:20 run was already replaced, so its logs are gone. If it persists with the new image, pull `kubectl logs` _during_ the QA window.
- **Prod namespace is now bare `yay-tsa`** (not `yay-tsa-v2-production`). Pods: `yay-tsa-v2-production-backend-*`, `yay-tsa-production-*` (frontend), `yay-tsa-production-audio-separator-*` all live in `kubectl -n yay-tsa`.
...
- **Controller-level error bodies returned `application/json`, not `application/problem+json` — 16 schemathesis "Undocumented Content-Type" failures, distinct from the 2026-05-27 security-filter fix.** The 2026-05-27 `ProblemDetailSecurityHandler` only covers 401/403 raised in the Spring Security _filter chain_. Error responses built _inside controllers_ are a separate layer: `HttpFailureTranslator.translate()` returned `.body(mapOf("error" to ...))` (→ `application/json`), and ~9 controllers (`JellyfinAdminController`, `JellyfinAuthController`, `JellyfinAdaptiveController`, `JellyfinDevicesController`, `JellyfinPlaylistsController`, `GroupController`) returned `mapOf("error" ...)` directly. The OpenAPI spec (`OpenApiConfig.globalResponsesCustomizer`) documents _only_ `application/problem+json` for 400/401/403/404/409/429/500, so every controller `application/json` error tripped schemathesis `content_type_conformance`. Fix: a shared `dev.yaytsa.adaptershared.problemDetail(status,title,detail)` helper emitting `application/problem+json` RFC7807, used by `HttpFailureTranslator.translate()` and every controller error site. The PWA reads `errorData.message` (falls back to `statusText`), never `.error`, so the body-shape change is display-safe. `GlobalExceptionHandler` was already correct (it set `APPLICATION_PROBLEM_JSON`); only the controller/translator layer deviated.
- **v2-ci (`v2-ci.yml`) Spotless failures silently freeze the backend image while the frontend keeps advancing.** A `spotlessKotlinCheck` violation in `infra-karaoke-worker` (shipped 2026-05-27 on `f02d7d0c`) made `v2 Gradle test` fail → `v2 Build Docker` skipped → no new backend image for ~2 days. Deployed backend sat on `main-0986c22` while frontend was on `main-19a5eb5`. **Always cross-check `kubectl get deployments -n yay-tsa -o jsonpath=...` per-image tags against HEAD** — a green `ci.yml` (frontend) does not imply the backend advanced; the backend builds in the separate `v2-ci.yml`. Run `./gradlew spotlessApply` before any backend push.
- **QA-account API auth (`yay-tsa-qa-password` keychain entry) returned 401 for username candidates `qa-autoqa@test.com`/`qa-autoqa`/`qa`** — password likely rotated or username differs. Authed manual QA is blocked locally; the authoritative authed coverage is the `post-deploy-qa` job (CI `QA_AUTH_TOKEN`). Do NOT use the `master` account visible via browser autofill — it is the owner/admin account and admin login is forbidden during `/qa`.

### QA Session Findings (2026-05-29, verification iteration)

- **`playback_signals.context` column type DRIFTS between test (jsonb) and prod (text)** — a false-confidence trap. Hibernate `ddl-auto` in the Testcontainers integration test creates `context` as **jsonb** (from the entity's JSON typing), but the production column is **text** (Flyway/v1-ETL origin). Consequence: the affinity-aggregator `total_listen_sec` SQL written with the jsonb `->>` operator **passes `AffinityWorkerIntegrationTest` but floods prod** with `BadSqlGrammarException: operator does not exist: text ->> unknown` every ~60s (the `@Scheduled` aggregation tick). The inverse (`substring(text ...)`) passes prod but fails the test with `function substring(jsonb, unknown) does not exist`. **Fix that works on BOTH: `substring(ps.context::text from '<regex>')`** — jsonb→text yields the JSON string, text→text is a no-op. Lesson: a green integration test does NOT prove prod-safe SQL when the test schema is Hibernate-generated and prod is Flyway-managed — they diverge on column types. Grep prod backend logs for `BadSqlGrammar`/`MessageBroker.*scheduled task` after any worker-SQL change.
- **Spring's default `/error` dispatch returns `application/json` (`{"timestamp","status","error","path"}`), and Tomcat connector-level URL rejections return `text/html`** — neither is `application/problem+json`, so both trip schemathesis `content_type_conformance` against the RFC7807-only OpenAPI spec. The `@RestControllerAdvice` only covers exceptions thrown _inside_ controller methods. Fix: a custom `ProblemDetailErrorController implements ErrorController` mapping `/error` → problem+json (Spring backs off `BasicErrorController` when a custom one exists). Note: truly malformed URL encodings (invalid 4-byte unicode like `%F2%95...`) are rejected by Tomcat _before_ the servlet error dispatch, so a few `text/html` 400s remain unfixable in app code — document, don't chase.
- **`post-deploy-qa` in `ci.yml` runs ~8 min after push, but the backend deploys ~30 min after push** (separate `v2-ci.yml` build → Forgejo gitops Image Updater → ArgoCD sync → rollout). So a given push's `post-deploy-qa` tests the **PREVIOUS** backend image, not the one just pushed. Always confirm `kubectl get deployment ... -o jsonpath='{...image}'` matches the SHA before trusting an authed schemathesis result. To verify a backend fix fast, poll the deployment image to the target SHA, then reproduce the specific endpoint with the `test` QA account token directly — don't wait a full post-deploy-qa cycle.
- **QA account: username is `test`** (the autoqa `auth-body`/`crawler-username`), password in keychain `yay-tsa-qa-password`. `AuthenticateByName` returns a Bearer `AccessToken`. The Subsonic `/rest/*` surface uses query-param auth (u/p/t), not Bearer, so a Bearer token yields 401 there — expected, not a bug.

### QA Session Findings (2026-05-29, correctness-audit fixes)

- **Affinity watermark "double-count fix" must NOT add a `created_at <= now()` upper bound.** The race the audit flagged (watermark read outside the upsert txn) is closed by `@Transactional` ALONE: PostgreSQL `now()` is transaction-start time and is stored as `updated_at`, so every processed signal's `created_at` is ≤ the new watermark — the next tick's strict `created_at > watermark` excludes it. Adding `AND ps.created_at <= now()` as an upper window edge _looks_ safer but silently drops future-dated / clock-skewed signals, and `AffinityWorkerIntegrationTest` deliberately seeds `base.plusSeconds(2)` (a future timestamp) on the second tick — the wall-clock `now()` boundary excludes it and the "accumulate across ticks" assert fails. Keep the `ps.context::text` cast (jsonb-test vs text-prod, see prior entry). Lesson: for watermark aggregators, atomicity (one txn) is the fix, not an artificial upper bound on event time.
- **Fail-closed path validation breaks the audio integration tests' implicit setup.** `AudioStreamIntegrationTest` writes fixtures via `Files.createTempFile(...)` (system `java.io.tmpdir`) and stores the absolute path in `source_path`. The OLD `MediaPathSafety` served any path when `safeRoot` was null (music-path unconfigured), so the tests passed by accident. After making `resolveServableFile` return null (404) on null `safeRoot`, set `registry.add("yaytsa.library.music-path") { System.getProperty("java.io.tmpdir") }` in `HttpIntegrationTestBase.@DynamicPropertySource` so the secured path serves the temp fixtures. `resolveRoot` and the candidate both `toRealPath()`, and tmpdir canonicalization (`/var/folders/...`→`/private/var/...`) matches on both sides, so `startsWith` holds. This is SUT configuration, not weakening assertions.
- **Verify schemathesis content-type failures against the LIVE image, not the failing CI log.** A failing `post-deploy-qa` showing ~13 "Undocumented Content-Type" + a few "API rejected schema-compliant request" on admin/playlist DELETEs was entirely against the pre-`problem+json` image (deploy lag). Reproducing each endpoint with the `test` token against the deployed tag showed `403 application/problem+json` and `404` (both in the accepted set) — already fixed. Always `curl -w '%{content_type}'` the deployed tag before treating a stale schemathesis log as live.
- **Browser QA without the forbidden `master` autofill:** the login form autofills the owner/admin `master` account. Bypass it by injecting a `test`-account `AccessToken` directly: `sessionStorage` keys `yaytsa_session` (token) + `yaytsa_user_id`, plus the `yay_token` cookie (`STORAGE_KEYS` in `packages/core/src/internal/config/constants.ts`), then navigate. No credential typing, no master login.
