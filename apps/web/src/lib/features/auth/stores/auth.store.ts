...
import {
  ...
  loadSessionAuto,
  clearAllSessions,
} from '../../../shared/utils/session-manager.js';
import { logger } from '../../../shared/utils/logger.js';
import { cacheManager } from '../../../cache/cache-manager.js';
import { setImageClient } from '../../../shared/utils/image.js';
...
async function logout(): Promise<void> {
  ...
  try {
    const playerModule = await import('../../player/stores/player.store.js');
  ...
  if ('caches' in window) {
    try {
      const cacheNames = ['yaytsa-images-0.0.0-placeholder', 'yaytsa-audio-0.0.0-placeholder'];
...
// Sync client to image utilities via dependency injection (avoids circular imports)
let imageClientUnsubscribe: (() => void) | null = null;
if (browser) {
  imageClientUnsubscribe = authStore.subscribe($auth => {
    setImageClient($auth.client);
  });

  if (import.meta.hot) {
    import.meta.hot.dispose(() => {
      if (imageClientUnsubscribe) {
        imageClientUnsubscribe();
        imageClientUnsubscribe = null;
      }
    });
  }
}
