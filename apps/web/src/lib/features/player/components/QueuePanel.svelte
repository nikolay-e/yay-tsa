<script lang="ts">
  ...
  function handleKeydown(event: KeyboardEvent) {
    ...
    // Focus trap: keep Tab navigation within the panel
  ...
  // Focus management for modal dialog - track transitions to avoid re-run on re-renders
  $: {
    if (isOpen && !wasOpen) {
...
{#if isOpen}
  <!-- Backdrop: click to close, keyboard handled via Escape on window -->
  <div class="queue-overlay" on:click={handleBackdropClick} role="presentation">
    <div
      class="queue-panel"
      role="dialog"
      aria-modal="true"
