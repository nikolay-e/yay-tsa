...
import { client } from '../auth/auth.store.js';
import { karaoke, shouldUseInstrumental } from './karaoke.store.js';
...
let karaokeStreamSwitchId = 0; // Tracks karaoke stream switch operations to prevent race conditions
...
if (browser) {
  ...
  // HMR cleanup - dispose subscriptions on module reload
  if (import.meta.hot) {
    import.meta.hot.dispose(() => {
      ...
      // Dispose all audio event handlers to prevent accumulation
      audioEventCleanups.forEach(cleanup => cleanup());
      audioEventCleanups = [];
      // Note: audioEngine and mediaSession are preserved across HMR
      // to avoid interrupting playback during development
    });
  }
}
...
async function playTrackFromQueue(track: AudioItem): Promise<void> {
  ...
  try {
    ...
    // Use module-level playbackState to avoid stale references from snapshot
    if (playbackState) {
      playbackState.setCurrentItem(track);
      playbackState.setStatus('playing');
      void playbackState.reportPlaybackStart(); // Fire-and-forget to avoid UI lag
    }
 ...
 * Uses operation ID tracking to prevent race conditions when rapidly pressing next
 */
async function next(): Promise<void> {
  // Create unique operation ID to detect if this operation is superseded
  const operationId = Symbol('next-operation');
  currentLoadOperation = operationId;
  ...
  if (nextTrack) {
    // Check if operation was superseded before starting load
    if (currentLoadOperation !== operationId) {
      return;
    }
    await playTrackFromQueue(nextTrack);
  } else {
    // No more tracks, stop playback (only if this operation is still current)
    if (currentLoadOperation === operationId) {
      stop();
    }
  }
 ...
 * Uses operation ID tracking to prevent race conditions when rapidly pressing previous
 */
async function previous(): Promise<void> {
  // Create unique operation ID to detect if this operation is superseded
  const operationId = Symbol('previous-operation');
  currentLoadOperation = operationId;
  ...
  if (timing.currentTime > RESTART_THRESHOLD_SECONDS) {
    // Only seek if this operation is still current
    if (currentLoadOperation === operationId) {
      seek(0);
    }
    return;
