...
// Returns null when the image yields no usable dominant color (all pixels filtered
// out, or the dominant color is too desaturated to theme around).
export function extractPaletteFromPixels(pixels: Uint8ClampedArray): ThemePalette | null {
  const rgb = extractDominantColor(pixels);
  if (!rgb) return null;
  const hsl = rgbToHsl(rgb.r, rgb.g, rgb.b);
  if (hsl.s < MIN_SATURATION) return null;
